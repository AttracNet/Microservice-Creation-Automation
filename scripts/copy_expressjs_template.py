...
def ignore_node_modules(dir, files):
    return set(files) if 'node_modules' in dir else set()
...
def copy_template(src_dir, dest_dir, force=False):
    try:
        ...
        shutil.copytree(src_dir, dest_dir, dirs_exist_ok=True, ignore=ignore_node_modules)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Copy Express.js template to a new microservice.")
    ...
    source_directory = "../templates/expressjs_template"
    ...
    # Ask for uer input to complete JSON config file
    ask_for_user_input(destination_directory, new_microservice)

    # Remove the 'node_modules' directory after the copy is completed
    node_modules_path = os.path.join(destination_directory, 'node_modules')
